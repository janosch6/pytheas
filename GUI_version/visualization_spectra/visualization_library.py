...
class Visualize:
    def __init__(
        ...
        self.digest_file, self.mgf_file, self.score_file, self.highest_peaks = (
            digest,
            mgf,
            match,
            high_peaks,
        ...
        self.visualize_decoys, self.only_modified, self.rank = (
            visualize_decoys,
            only_modified,
            rank,
        )
        self.only_unique_positions, self.redundant_x = (
            unique_positions,
            remove_redundant_x,
        )
    ...
    def digest_peaks(self, input_file):
        ...
        for ion in precursors:
            outlist.append(
                "BEGIN IONS\nprecursor= {} seq= {} mod= {} charge= {}\n".format(
                    ion.split("_")[0], ion.split("_")[1], dic[ion][7], dic[ion][4]
                )
            )
            ...
            for i in MS2_ions:
                outlist.append(
                    str(round(np.float64(i.split(":")[-1]), 6))
                    + "\t100\t"
                    + i.split(":")[0]
                    + "\n"
                )
        ...
        for line in outlist:
            ...
            if "precursor" in line:
                flag, prec_mass, seq, charge = (
                    1,
                    line.split("=")[1].split()[0],
                    line.split("=")[2].split()[0],
                    line.split("=")[4].split()[0],
                )
                out_dic[prec_mass + "_" + seq + "_" + charge] = {
                    "m/z": [],
                    "intensity": [],
                    "ion": [],
                }
            ...
            if flag == 1 and line[0].isdigit():
                (
                    out_dic[prec_mass + "_" + seq + "_" + charge]["m/z"].append(
                        np.float64(line.split("\t")[0])
                    ),
                    out_dic[prec_mass + "_" + seq + "_" + charge]["intensity"].append(
                        np.float64(line.split("\t")[1])
                    ),
                    out_dic[prec_mass + "_" + seq + "_" + charge]["ion"].append(
                        line.split("\t")[2][:-1]
                    ),
                )
    ...
    def df_for_dSp2(self):
        ...
        if not os.path.exists(self.score_file):
            print(
                "File {} does not exist. Execution terminated without output".format(
                    self.score_file
                )
            )
            sys.exit(1)
        ...
        with open(self.score_file, "r") as input_file:
            for line in input_file:
                if line[0].isdigit():
                    ...
                    d["m/z"].append(mz), d["RT"].append(rt), d["m/z_RT"].append(
                        "{}_{}".format(mz, rt)
                    )
                    d["sequence"].append(sequence), d["rank"].append(rank), d[
                        "Sp"
                    ].append(score)
    ...
    def scored_info(self, input_file):
        ...
        for line in open(input_file, "r"):
            ...
            if flag == 1 and line[0].isdigit():
                ...
                rank, offset = int(split[6].split("=")[1]), abs(
                    float(split[3].split("p")[0])
                )

                # Apply all the cutoffs and conditions from user-selected options
                if (
                    float(score) >= self.Sp_cutoff
                    and float(dsp) <= self.dSp_cutoff
                    and decoys_string not in molecule
                    and mod_string in mod
                    and len(unique_list) <= unique_length
                    and rank <= self.rank
                    ...
                    MS2_scans, charge, sequence, positions = (
                        split[7].split("=")[1],
                        split[10],
                        split[11],
                        split[15],
                    ...
                    if self.redundant_x:
                        if "X" in sequence:
                            if (
                                "{}_{}_{}".format(sequence, prec_ion, rt)
                                not in sequences_with_x
                            ):
                                sequences_with_x.append(
                                    "{}_{}_{}".format(sequence, prec_ion, rt)
                                )
                            else:
                                flag_x = True
                    ...
                    if not flag_x:
                        ...
                        for scan in split[16:-1]:
                            mgf_peak, relative_intensity, th_peak, ion = (
                                scan.split(":")[0].split("(")[0],
                                scan.split("]")[0].split("[")[1],
                                scan.split(":")[1].split("[")[0],
                                scan.split("[")[2].split("]")[0],
                            )
                            dic[prec_ion][unique_name]["MS2_match"].append(
                                (mgf_peak, relative_intensity, th_peak, ion)
                            )
    ...
    def df_output_html(self):
                ...
                if line[0].isdigit():
                    ...
                    if (
                        ...
                        if self.redundant_x:
                            if "X" in seq:
                                if (
                                    "{}_{}_{}".format(seq, unique_precursor, rt)
                                    not in sequences_with_x
                                ):
                                    sequences_with_x.append(
                                        "{}_{}_{}".format(seq, unique_precursor, rt)
                        ...
                        if not flag_x:
                            ...
                            d["m/z"].append(
                                '<a href="#{}_{}_{}_{}">{}</a>'.format(
                                    mz_isotopologues_correction, rt, score, seq, mz
                                )
                            )
                            d["m/z_RT"].append(
                                "{}_{}".format(mz_isotopologues_correction, rt)
                            )

                            # Insert all the other info for all ions in preparation for the table output
                            d["Rank"].append(sp[6].split("=")[1])
                            d["RT"].append(rt), d["Score (Sp)"].append(score), d[
                                "dSp"
                            ].append(dSp),
                            d["charge"].append(sp[10]), d["isotope"].append(sp[8]), d[
                                "molecule_location"
                            ].append(sp[15]),
                            d["sequence"].append(seq), d["sequence_mods"].append(
                                sp[12]
                            ),
                            d["5'-end"].append(sp[13]), d["3'-end"].append(sp[14]),
                            d["MS1_offset(ppm)"].append(sp[3].split("p")[0]), d[
                                "length"
                            ].append(len(seq)),
                            d["n/L"].append(
                                round(
                                    int(sp[-1].split(";")[1].split("=")[1])
                                    / int(sp[-1].split(";")[-2].split("=")[1]),
                                    3,
                                )
                            )
        ...
        return df.to_html(
            classes="sortable", justify="center", escape=False, table_id="output_table"
        )
    ...
    def scored_spectra(self, match):
        ...
        if match[1]["MS2_match"] and np.float64(match[1]["score"]) > 0:
            ...
            key_mgf = (
                re.findall("[\d]+[.]?[\d]+", match[0].split("_")[0])[0]
                + "_"
                + match[0].split("_")[1]
            )

            MS2_matches, charge, prec, mz_highest_ion = (
                match[1]["MS2_match"],
                int(re.findall(r"\d+", match[1]["charge"])[0]),
                np.float64(re.findall("[\d]+[.]?[\d]+", match[0].split("_")[0])[0]),
                0,
            )
            ...
            fig = plt.figure(
                1, figsize=(conv_inch(self.figwidth_mm), conv_inch(self.figheight_mm))
            )
            ax = fig.add_subplot(
                111, autoscale_on=False, xlim=(x_min, x_max), ylim=(0, y_max)
            )
            ...
            if self.highest_peaks != "all":
                ...
                red_dic = input_mgf[key_mgf].nlargest(
                    len(input_mgf[key_mgf]), "intensity"
                )
                limit_peaks = 99999
            ...
            # multiple times
            unique_digest_table = copy.copy(
                dig_tab[match[1]["th_match"] + "_" + match[1]["sequence"]]
            )
                ...
                if index <= limit_peaks:
                    ...
                    if m:
                        ...
                        if (
                            ...
                            unique_digest_table.loc[int(m.group()) - 1, ion_series] = (
                                str(round(np.float64(i[2]), 4)) + "_" + i[3][0] + "clr"
                            )

                        else:
                            unique_digest_table.loc[
                                len(match[1]["sequence"]) - int(m.group()), ion_series
                            ] = (
                                str(round(np.float64(i[2]), 4)) + "_" + i[3][0] + "clr"
                            )
                    ...
                    if re.search(r"^(?![abcdzyxw])", i[3].split("(")[0].split("-")[0]):
                        unique_digest_table.loc[0, i[3]] = (
                            str(round(np.float64(i[2]), 4)) + "_Mclr"
                        )
                    ...
                    if (
                        input_mgf[key_mgf]
                        .loc[input_mgf[key_mgf]["m/z"] == prec_fl]
                        .empty
                    ):
                        prec_fl += 0.000001
                    ...
                    if (
                        ...
                        y_lim_annotation = int(
                            input_mgf[key_mgf]
                            .loc[input_mgf[key_mgf]["m/z"] == prec_fl, "intensity"]
                            .iloc[0]
                        )

                        # Set the annotation for ions to be shown even when they are offscale
                        if y_lim_annotation > y_max:
                            y_coord_ions, x_coord_ions = (
                                y_max * 0.95,
                                np.float64(i[0]) * 1.02 + self.bars_width * 4,
                            )
                        else:
                            y_coord_ions, x_coord_ions = (
                                y_lim_annotation * 1.01,
                                np.float64(i[0]),
                            )

                        if i[3][0] in color_MS2.keys() and re.search(
                            r"\d+$", i[3].split("(")[0].split("-")[0]
                        ):
            ...
            plt.savefig(
                "./scored_spectra_"
                + filename_from_path(self.mgf_file)[:-4]
                + "/"
                + key_mgf
                + "_"
                + match[1]["score"]
            ...
            self.html_lines.append(
                """<div class="wrapper">
                                  ...
                                  """.format(
                    key_mgf, match[1]["score"], match[1]["sequence"]
                )
            )
    ...
    def final_output(self):
        ...
        for key in input_scored:
            ...
            d = input_scored[key]
            score_sorted = sorted(
                d.items(), key=lambda x: np.float64(x[1]["score"]), reverse=True
            )
...
def plot_limits(xy_dataframe, dic_key):
    ...
    """
    return (
        round_number(min(xy_dataframe[dic_key]["m/z"])) - 25,
        round_number(max(xy_dataframe[dic_key]["m/z"])) + 25,
        round_number(max(xy_dataframe[dic_key]["intensity"])) + 50,
    )
...
def annotation(row, ax, bars_width):
    ...
    """
    ax.annotate(
        row["ion"] + " " + str(round(row["m/z"], 3)),
        xy=(row["m/z"], 105),
        xycoords="data",
        xytext=(row["m/z"] - bars_width * 4, 130),
        textcoords="data",
        rotation=90,
        color=color_MS2[row["ion"][0]],
        size=5,
    )
        ...
        for ion in dic[precursor][12:]:
            ...
            if m:
                ...
                if ion[0] == "a" or ion[0] == "b" or ion[0] == "c" or ion[0] == "d":
                    d[ion_series][int(m.group()) - 1] = str(
                        round(np.float64(ion.split(":")[1]), 4)
                    )

                else:
                    d[ion_series][seq_length - int(m.group())] = str(
                        round(np.float64(ion.split(":")[1]), 4)
                    )

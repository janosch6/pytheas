...
class Stats:
    def __init__(
        ...
        box,
        scatter_Sp_vs_dSp,
        scatterbox_lengths_z,
    ):
        self.targets_csv, self.decoys_csv, self.match_file, self.light_heavy = (
            targets,
            decoys,
            match,
            isotope,
        ...
        self.scatter_Sp_dSp, self.scatterbox_lengths_z = (
            scatter_Sp_vs_dSp,
            scatterbox_lengths_z,
        )
    ...
    def graphs_plot(self):
        ...
        """
        csv_files = (
            read_csv_input(self.targets_csv, self.light_heavy),
            read_csv_input(self.decoys_csv, self.light_heavy),
        )

        if csv_files[0].empty:
            print(
                "ERROR! CSV input file {} is empty. Execution terminated without generating any output".format(
                    self.targets_csv
                )
            )
            sys.exit(1)
        ...
        plt.subplot(2, 1, 1)
        stats.ppm_errors_histogram(
            csv_files, self.Sp_cutoff, "MS1", self.match_file, bbox
        )

        plt.subplot(2, 1, 2)
        stats.ppm_errors_histogram(
            csv_files, self.Sp_cutoff, "MS2", self.match_file, bbox
        )
        ...
        plt.subplot(2, 1, 1)
        stats.scatter_nts_vs_score(
            csv_files, "Sp", 0, 0, lengths, self.Sp_cutoff, bbox, targets_decoys
        )
        plt.subplot(2, 1, 2)
        stats.box_nts_vs_score(
            csv_files, "Sp", 0, 0, lengths, self.Sp_cutoff, targets_decoys
        )
        ...
        if self.scatterbox_lengths_z:
            try:
                plt.subplot(2, 1, 1)
                stats.scatter_nts_z_vs_score(
                    csv_files, "Sp", 0, 0, lengths, self.Sp_cutoff, targets_decoys, bbox
                )
                plt.subplot(2, 1, 2)
                stats.box_nts_z_vs_score(
                    csv_files, "Sp", 0, 0, lengths, self.Sp_cutoff, targets_decoys
                )
                ...
                print(
                    "Scatter + boxplot of Sp scores of matches vs length grouped by charge could "
        ...
        if not csv_files[1].empty:
            ...
            if self.scatter_Sp_dSp:
                stats.scatter_dSp_vs_Sp(
                    csv_files, lengths, self.Sp_cutoff, bbox, targets_decoys
                )
    ...
    if os.path.exists(namefile):
        ...
        for n in numbers:
            if not os.path.exists(
                "_".join(namefile.split("_")[:-1]) + "_{}.png".format(n)
            ):
                plt.savefig(
                    "_".join(namefile.split("_")[:-1]) + "_{}.png".format(n),
                    bbox_inches="tight",

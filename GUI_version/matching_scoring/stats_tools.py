...
def input_data(infile, excel, sumi_all):
    ...
    with open("./" + infile, "r") as input_file:
        for line in input_file:
            ...
            if line[0].isdigit():
                ...
                d["isotope"].append(sp[8]), d["molecule_location"].append(sp[15]), d[
                    "sequence"
                ].append(sp[11]),
                d["sequence_mods"].append(sp[12]), d["RT"].append(sp[1].split("=")[1]),
                d["m/z_RT"].append(prec_M + "_" + sp[1].split("=")[1]), d[
                    "MS1_ppm"
                ].append(sp[3][:-3]),
                ...
                for match in sp[16:-1]:
                    MS2_ppm_list.append(
                        match.split(":")[0].split(")")[0].split("(")[1][:-3]
                    )
    ...
    if excel != "n":
        ...
        for index, row in df.iterrows():
            ...
            max_int = df_excel[0][
                (df_excel[0]["m/z obs"].str.contains(row["m/z"]))
                & (df_excel[0]["RT"].str.contains(row["RT"]))
            ]["Prec_max_int"]
            for m in max_int:
                df.loc[df.index[index], "Prec_max_int"] = m

            max_int = df_excel[1][
                (df_excel[1]["m/z obs"].str.contains(row["m/z"]))
                & (df_excel[1]["RT"].str.contains(row["RT"]))
            ]["Prec_max_int"]
            for m in max_int:
                df.loc[df.index[index], "Prec_max_int"] = m
...
def filter_data(df, top, yl):
    ...
    """
    if top == "y":
        filt_df = df.sort_values(["m/z", yl], ascending=[True, False]).reset_index(
            drop=True
        )
        ...
        if filt_df.empty:
            print(
                "ERROR!!!!!! No target matches found. Execution terminated without output"
            )
            sys.exit(1)
        ...
        decoy_df = decoy_df.sort_values(
            ["Sp", "rank"], ascending=[False, True]
        ).drop_duplicates(subset=["m/z", "RT"])
        decoy_df = decoy_df.sort_values(
            ["m/z", yl], ascending=[True, False]
        ...
        dup_df = dup_df.sort_values("Sp", ascending=False).drop_duplicates(
            subset=["m/z", "RT"]
        )
        ...
        filt_df = filt_df.sort_values(["m/z", yl], ascending=[True, False]).reset_index(
            drop=True
        )
...
def csv_output(df, lengths, per, Sp_cutoff, name_output, targets_with_decoys, isotopes):
    ...
    if isotopes == "light":
        in_plot_df, in_decoy_df = (
            in_plot_df[in_plot_df["isotope"] == "light"],
            in_decoy_df[in_decoy_df["isotope"] == "light"],
        )
    elif isotopes == "heavy":
        in_plot_df, in_decoy_df = (
            in_plot_df[in_plot_df["isotope"] == "heavy"],
    ...
    if per == "y":
        ...
        nts, d = ["all"] + sorted(plot_df["length"].unique()), {
            "length": [],
            "99th": [],
            "95th": [],
            "90th": [],
        ...
        for i in nts:
            ...
            if i != "all":
                d["99th"].append(
                    len(
                        plot_df.loc[(plot_df["Sp"] > p99) & (plot_df["length"] == i)][
                            "m/z"
                        ]
                    )
                ), d["95th"].append(
                    len(
                        plot_df.loc[(plot_df["Sp"] > p95) & (plot_df["length"] == i)][
                            "m/z"
                        ]
                    )
                )
                d["90th"].append(
                    len(
                        plot_df.loc[(plot_df["Sp"] > p90) & (plot_df["length"] == i)][
                            "m/z"
                        ]
                    )
                ), d["80th"].append(
                    len(
                        plot_df.loc[(plot_df["Sp"] > p80) & (plot_df["length"] == i)][
                            "m/z"
                        ]
                    )
                )
                d["75th"].append(
                    len(
                        plot_df.loc[(plot_df["Sp"] > p75) & (plot_df["length"] == i)][
                            "m/z"
                        ]
                    )
                ), d["50th"].append(
                    len(
                        plot_df.loc[(plot_df["Sp"] > p50) & (plot_df["length"] == i)][
                            "m/z"
                        ]
                    )
                )
                d["0th"].append(
                    len(
                        plot_df.loc[(plot_df["Sp"] > p0) & (plot_df["length"] == i)][
                            "m/z"
                        ]
                    )
                )
                ...
                d["99th"].append(len(plot_df.loc[plot_df["Sp"] > p99]["M"])), d[
                    "95th"
                ].append(len(plot_df.loc[plot_df["Sp"] > p95]["m/z"])), d[
                    "90th"
                ].append(
                    len(plot_df.loc[plot_df["Sp"] > p90]["m/z"])
                )
                d["80th"].append(len(plot_df.loc[plot_df["Sp"] > p80]["m/z"])), d[
                    "75th"
                ].append(len(plot_df.loc[plot_df["Sp"] > p75]["m/z"])), d[
                    "50th"
                ].append(
        ...
        percentile_df.to_csv(
            "./Sp_vs_percentileSpdecoys_{}.csv".format(name_output), index=False
        )

    label_lengths = "all"
    # Apply the filter on sequence lengths for FDR estimation
    if lengths != "all":
        plot_df = in_plot_df.loc[
            (in_plot_df["length"].isin(lengths)) & (in_plot_df["Sp"] > Sp_cutoff)
        ]
    ...
    merged_df = pd.concat(
        [
            targets_FDR_df.loc[targets_FDR_df["rank"] == 1],
            decoy_df.loc[decoy_df["rank"] == 1],
        ],
        ignore_index=True,
        sort=True,
    )
    ...
    decoy_max = np.float64(
        merged_df[merged_df["molecule_location"].str.contains("decoy") == True][
            "Sp"
        ].max()
    )
    ...
    if not np.isnan(decoy_max):
        ...
        for i in np.arange(0, round(decoy_max, 3) + step, step):
            ...
            if tot > 0:
                ...
                top_decoy = len(
                    merged_df[
                        (merged_df["molecule_location"].str.contains("decoy") == True)
                        & (merged_df["Sp"] >= i)
                        & (merged_df["rank"] == 1)
                    ]["m/z"]
                )
                d["top_match_decoys(D)"].append(top_decoy), d["FDR(D/t)"].append(
                    round(top_decoy / tot, 4)
                ), d["FDR(%)"].append(round(top_decoy * 100 / tot, 2))
        ...
        d = {
            "Sp_cutoff": [0],
            "top_match_targets(t)": [len(merged_df[merged_df["Sp"] > 0]["m/z"])],
            "top_unique_targets": [toptargets_df["sequence"].nunique()],
            "top_match_decoys(D)": [0],
            "FDR(D/t)": [0],
            "FDR(%)": [0],
        }
    ...
    targets_w_decoys, targets_without_decoys = len(targets_w_decoys_df.index), len(
        targets_FDR_df.index
    ) - len(targets_w_decoys_df.index)
    open("FDR_{}.csv".format(name_output), "w").writelines(
        [
            ...
            "Sequence lengths targets/decoys,{}\n".format(
                targets_w_decoys, targets_without_decoys, label_lengths
            ),
            outcsv,
        ]
    )
...
def scatter_Sp_vs_Spdecoy(df, lengths, Sp_cutoff, info_box):
    ...
    if "all" in lengths:
        ...
        fig.suptitle(
            "Best targets vs corresponding decoys Sp scores at different precursor lengths",
            y=0.95,
            fontsize=7,
            fontweight="bold",
        )
        ...
        for i, l in enumerate(nts):
            ...
            axs[graphs[i][0], graphs[i][1]].scatter(
                x,
                y,
                s=10,
                facecolors="dimgray",
            ...
            axs[graphs[i][0], graphs[i][1]].set_title(
                "{}-mer[{}]".format(l, x.count()), pad=-5, fontsize=6
            )
            for ax in axs.flat:
            ...
            # Plot the line x = y
            axs[graphs[i][0], graphs[i][1]].plot(
                list(range(100)),
                list(range(100)),
                color="black",
                linewidth=0.1,
    ...
    if "analysis" not in lengths:
        ...
        plt.scatter(
            x1, y1, facecolors="dimgray", edgecolors="black", marker=".", alpha=0.7
        )
...
def scatter_dSp_vs_Sp(df, lengths, Sp_cutoff, info_box, targets_with_decoys):
    ...
    plt.scatter(
        x1, y1, facecolors="royalblue", edgecolors="blue", marker=".", alpha=0.75
    )
    plt.scatter(
        x2, y2, facecolors="lightcoral", edgecolors="red", marker=".", alpha=0.75
    )
    ...
    # Create a legend
    dec_legend = plt.scatter(
        [], [], facecolors="lightcoral", edgecolors="red", marker=".", alpha=0.75
    )
    tar_legend = plt.scatter(
        [], [], facecolors="royalblue", edgecolors="blue", marker=".", alpha=0.75
    )
    plt.legend(
        (tar_legend, dec_legend),
        ("targets ({})".format(len(x1.index)), "decoys ({})".format(len(x2.index))),
    )


def scatter_nts_vs_score(
    df, yl, y_bot, y_top, lengths, Sp_cutoff, info_box, targets_with_decoys
):
    """
    ...
    if not plot_df.empty:
        ...
        if y_bot and y_top != 0:
            ...
            if y_top != 0:
                ...
                plt.ylim(
                    y_bot - plot_df[yl].max() / 50,
                    plot_df[yl].max() + plot_df[yl].max() / 25,
                )
    ...
    if not plot_df.empty:
        ...
        if y_bot and y_top != 0:
            ...
            if y_top != 0:
                ...
                plt.ylim(
                    (
                        y_bot - plot_df["Sp"].max() / 50,
                        plot_df["Sp"].max() + plot_df["Sp"].max() / 25,
                    )
                )
        ...
        if "all" in lengths:
            ...
            lengths_labels = list(
                np.arange(plot_df["length"].min(), plot_df["length"].max() + 1)
            )

            target = plt.boxplot(
                data1,
                positions=[np.float64(x) - 0.15 for x in lengths_labels],
                manage_ticks=False,
                widths=0.25,
                sym="b.",
            )
            ...
            target = plt.boxplot(
                data1,
                positions=[np.float64(x) - 0.15 for x in lengths],
                manage_ticks=False,
                widths=0.25,
                sym="b.",
            )
...
def hist_Sp(df, lengths, Sp_cutoff, targets_with_decoys, info_box):
    ...
    median_t, mean_t, std_t = (
        plot_df["Sp"].median(),
        plot_df["Sp"].mean(),
        plot_df["Sp"].std(),
    )
    median_d, mean_d, std_d = (
        decoy_df["Sp"].median(),
        decoy_df["Sp"].mean(),
        decoy_df["Sp"].std(),
    )
    ...
    if not plot_df.empty:
        ...
        plt.hist(
            plot_df["Sp"],
            bins=50,
            range=(0, maxvalue + 0.1),
            alpha=0.5,
            label="targets ({})".format(len(plot_df.index)),
            edgecolor="black",
            color="blue",
        )
        ...
        plt.hist(
            ...
            range=(0, maxvalue + 0.1),
            alpha=0.5,
            label="decoys ({})".format(len(decoy_df.index)),
            edgecolor="black",
            color="red",
        )
        ...
        if info_box == "y":
            textstr = "\n".join(
                (
                    ...
                    "Targets w/ decoys = {}".format(targets_with_decoys),
                    "sequence lengths = {}".format(label_lengths),
                    r"$\mu(t)=%.2f$ M(t)=%.2f $\sigma(t)=%.2f$"
                    % (
                        mean_t,
                        median_t,
                        std_t,
                    ),
                    r"$\mu(d)=%.2f$ M(d)=%.2f $\sigma(d)=%.2f$"
                    % (
                        mean_d,
                        median_d,
                        std_d,
...
def hist_top_Sp(df, lengths, Sp_cutoff, targets_with_decoys, info_box):
    ...
    top_plot_df, top_decoy_df = (
        plot_df[plot_df["rank"] == 1],
        decoy_df[decoy_df["rank"] == 1],
    )
    ...
    # Determine statistical descriptors for targets and decoys
    median_t, mean_t, std_t = (
        top_plot_df["Sp"].median(),
        top_plot_df["Sp"].mean(),
        top_plot_df["Sp"].std(),
    )
    median_d, mean_d, std_d = (
        top_decoy_df["Sp"].median(),
        top_decoy_df["Sp"].mean(),
        top_decoy_df["Sp"].std(),
    )
    ...
    if not plot_df.empty:
        ...
        plt.hist(
            ...
            range=(0, maxvalue + 0.1),
            alpha=0.5,
            label="targets ({})".format(len(top_plot_df.index)),
            edgecolor="black",
            color="blue",
        )
        plt.hist(
            ...
            range=(0, maxvalue + 0.1),
            alpha=0.5,
            label="decoys ({})".format(len(top_decoy_df.index)),
            edgecolor="black",
            color="red",
        )
...
def hist_second_dSp(df, lengths, Sp_cutoff, targets_with_decoys, info_box):
    ...
    # Determine statistical descriptors for targets and decoys
    median_t, mean_t, std_t = (
        sec_plot_df["dSp2"].median(),
        sec_plot_df["dSp2"].mean(),
        sec_plot_df["dSp2"].std(),
    )

    plt.xlabel("dSp"), plt.ylabel("Frequency")
    plt.title(
        "dSp2 scores distribution of top targets (rank = 1) with at least another competing target"
    )
    ...
    if not plot_df.empty:
        plt.hist(
            ...
            bins=20,
            alpha=0.5,
            label="targets ({})".format(len(sec_plot_df.index)),
            edgecolor="black",
            color="blue",
        )
...
def ppm_errors_histogram(df, Sp_cutoff, label, match_file, info_box):
    ...
    median_MS1, mean_MS1, std_MS1 = (
        plot_df["MS1_ppm"].median(),
        plot_df["MS1_ppm"].mean(),
        plot_df["MS1_ppm"].std(),
    )
    median_MS2, mean_MS2, std_MS2 = (
        statistics.median(MS2_ppm_list),
        statistics.mean(MS2_ppm_list),
        statistics.stdev(MS2_ppm_list),
    )
    ...
    plt.title(
        "m/z offsets (ppm) for the top {} targets (centered around the input offset)".format(
            label
        )
    )
    ...
    if label == "MS1":
        ...
        plt.hist(
            MS2_ppm_list,
            bins=50,
            label="MS2 ions ({})".format(len(MS2_ppm_list)),
            range=[-MS2_ppm, MS2_ppm],
            edgecolor="black",
            color="red",
        )
...
def scatter_nts_z_vs_score(
    df, yl, y_bot, y_top, lengths, Sp_cutoff, targets_with_decoys, info_box
):
    """
    ...
    # Apply the Sp cutoff
    plot_df, decoy_df = (
        inplot_df.loc[inplot_df["Sp"] > Sp_cutoff],
        indecoy_df.loc[indecoy_df["Sp"] > Sp_cutoff],
    )
    ...
    if not plot_df.empty:
        ...
        plt.title(
            "Sp score vs sequence length & charge for best targets & competing decoys"
        )
...
def FDR_update(df, lengths, Sp_cutoff, name_output):
    ...
    targets_with_decoys, targets_without_decoys = len(targets_w_decoys_df.index), len(
        targets_FDR_df.index
    ) - len(targets_w_decoys_df.index)
    open("FDR_{}_edited.csv".format(name_output), "w").writelines(
        [
            "Targets with competing decoys used for FDR,{}\nTargets without competing decoys used for FDR,{}\n"
            "Sequence lengths targets/decoys,{}\n".format(
                targets_with_decoys, targets_without_decoys, label_lengths
            ),
            outcsv,
        ]
    )

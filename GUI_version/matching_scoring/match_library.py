...
class Match:
    def __init__(
        ...
        self.MS1_mzlow, self.MS1_mzhigh, self.MS2_mzlow, self.MS2_mzhigh = (
            MS1_mzmin,
            MS1_mzmax,
            MS2_mzmin,
            MS2_mzmax,
        )
        self.MS1_ppm, self.MS2_ppm, self.MS1_ppm_offset = (
            round(MS1_ppm, 1),
            round(MS2_ppm, 1),
            -round(MS1_offset_ppm, 1),
        )
        self.MS2_ppm_offset = -round(MS2_offset_ppm, 1)
        self.MS2_peak_int_min, self.MS2_peak_num_max = MS2_intensity_min, MS2_peaks_max
        self.precursor_window_removal, self.losses_window_removal = (
            precursor_removal,
            loss_removal,
        )
        self.beta_increment, self.alpha, self.MS2_normint_cutoff = (
            beta,
            alpha,
            MS2_cutoff_normalized_int,
        )
        self.precursor_isotopologues, self.ignore_charges_mgf = (
            isotopologues,
            charges_mgf,
        )
        self.FDR_light_heavy, self.targets_without_decoys = (
            FDR_isotopes,
            targets_with_decoys,
        )
        ...
        for key in match_peaks:
            if match_peaks[key]:
                ...
                for t in match_peaks[key]:
                    ...
                    for f in mgf_peaks[key]:
                        ...
                        for peak in digest[t][-1]:
                            ...
                            mz_digest, mz_mgf, charge = (
                                ...
                                + ppm_range(
                                    np.float64(peak.split(":")[-1]), self.MS2_ppm_offset
                                ),
                                np.float64(f),
                                int(
                                    re.findall(
                                        r"\d+", peak.split("(")[1].split(")")[0]
                                    )[0]
                                ),
                            )
                            ...
                            if (
                                mz_mgf < self.MS2_mzlow
                                or mz_mgf > self.MS2_mzhigh
                                or (
                                    mz_digest - ppm_range(mz_digest, self.MS2_ppm)
                                    <= mz_mgf
                                    <= mz_digest + ppm_range(mz_digest, self.MS2_ppm)
                                )
                                or (
                                    mz_digest
                                    - ppm_range(mz_digest, self.MS2_ppm)
                                    + neutron_mass / charge
                                    <= mz_mgf
                                    <= mz_digest
                                    + ppm_range(mz_digest, self.MS2_ppm)
                    ...
                    output_dic[key].update(
                        {t: match_peaks[key][t][:7] + normalized_list}
                    )
                        ...
                        if "M-" not in ion[1] and len(ion[1].split("(")[0]) != 1:
                            ...
                            for mass_loss in mass_losses_list:
                                ...
                                mass_loss_offseted = mass_loss + ppm_range(
                                    mass_loss, self.MS2_ppm_offset
                                )

                                # Check and discard any sequencing ion is found in the M-xx exclusion window
                                if (
                                    mass_loss_offseted - self.losses_window_removal
                                    <= mz_ion
                                    <= mass_loss_offseted + self.losses_window_removal
                                ):
                                    flag = 0
        ...
        for i in range(1, 20):
            if len(ion_series) == 4:
                if (
                    ...
                    beta += (
                        self.beta_increment
                        + (self.alpha * consec_flag) * self.beta_increment
                    )
                    consec_flag += 1
                    ...
                    if list_mz_MS2:
                        ...
                        for line in list_mz_MS2:
                            ...
                            if (
                                ...
                                and np.float64(l[1]) > self.threshold_MS2_int()
                                and (
                                    np.float64(l[0])
                                ...
                                d[prec_mass + "_" + rt + "_" + charge].update(
                                    {l[0]: l[1]}
                                )
            ...
            for k in d[key]:
                ...
                # Adds free bases ions even if they are outside the specified MS2 window
                    ...
                    for ion in mz_freeb:
                        if (
                            np.float64(ion)
                            + ppm_range(np.float64(ion), self.MS2_ppm_offset)
                            - ppm_range(
                                np.float64(ion)
                                + ppm_range(np.float64(ion), self.MS2_ppm_offset),
                                self.MS2_ppm,
    ...
    def matching(self):
        ...
        """
        digest_dic, mgf_dic = (
            self.digest_dic(open(self.digest_file, "r")),
            self.mgf_dic(),
        )
            ...
            for key_match in prec_match:
                ...
                for x in prec_match[key_match][-1]:
                    ...
                    add = list(
                        ...
                        - ppm_range(
                            ion_mass + ppm_range(ion_mass, -self.MS2_ppm_offset),
                            self.MS2_ppm,
                        )
                        <= np.float64(k)
                        <= ion_mass
                ...
                for a in map(list, mgf_dic[key].items()):
                    if a[0] not in matched_ions.keys():
                        ...
                        all_mgf_list.append(
                            [self.MS2_mzlow + flag, matched_ions[a[0]]] + a
                        )
        ...
        for key_rt in d:
            ...
            for key in d[key_rt]:
                ...
                sumi, MS2_scans, gamma = (
                    self.sumI(d[key_rt][key][7:], mod_nts),
                    d[key_rt][key][7:],
                    0,
                )
                sumi_all = self.sumI(d_all[key_rt][key][7:], mod_nts)

                n, l = self.n_calc(d[key_rt][key][7:], mod_nts), self.L(
                    key, dig, mod_nts
                )
    ...
    def output(self):
        ...
        output_lines = [
            ...
            + "\n#MS2_mz_maximum "
            + str(self.MS2_mzhigh)
            + "\n#MS2_abs_peak_intensity "
            + str(self.MS2_peak_int_min)
            + "\n#MS2_peak_num_maximum "
            + str(self.MS2_peak_num_max)
            + "\n#MS2_normint_cutoff "
            + str(self.MS2_normint_cutoff)
            + "\n#precursor_exclusion_window "
            + str(self.precursor_window_removal)
            + "\n#precursor_losses_exclusion_window "
            + str(self.losses_window_removal)
            + "\n#alpha "
            + str(self.alpha)
            + "\n#beta "
            + str(self.beta_increment)
            + "\n#precursor_isotopologues "
            + str(self.precursor_isotopologues)
            + "\n"
        ]
        output_lines.append(
            ...
            if ctrl:
                ...
                for ion_rt in match_dic[key]:
                    ...
                    for match in ion_rt[1]:
                        ...
                        if abs(float(corrected_offset[:-1])) < self.MS1_ppm:
                            prec_count += 1
                            line_list = [
                                ion_rt[0].split("_")[0],
                            ...
                            for i, MS2 in enumerate(ion_rt[1][match][6:-1]):
                                ...
                                theoretical_mass = np.float64(
                                    ion_rt[1][match][i + 7][0]
                                )
                                if (
    ...
    def final_output(self):
        ...
        stats.csv_output(
            stats.filter_data(
                stats.input_data("match_output_" + mgf_name + ".txt", "n", "y"),
                "y",
                "Sp",
            ),
            "all",

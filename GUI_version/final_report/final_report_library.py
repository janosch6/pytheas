...
class Pytheas_Output:
    def __init__(
        ...
        visualize_decoys,
        only_modified,
        only_unique_positions,
        rank,
        redundant_x,
        dSp2_cutoff,
        MS1_offset,
    ):
        self.match_file, self.Sp_cutoff, self.dSp_cutoff = (
            match_file,
            Sp_cutoff,
            dSp_cutoff,
        )
                ...
                if line[0].isdigit():
                    ...
                    if (
                        ...
                        if not flag_x:
                            ...
                            d["sequence_location"].append(sp[15]), d["sequence"].append(
                                seq
                            ),
                            d["sequence_mods"].append(sp[12]), d["5'-end"].append(
                                sp[13]
                            ), d["3'-end"].append(sp[14]),
                            ...
                            if (
                                mod != "-"
                                and "X" not in seq
                                and "decoy" not in molecule
                            ):
                                d["modifications_positions"].append(
                                    modification_position(molecule, seq, mod)
                                )
                            else:
                                d["modifications_positions"].append("")
    ...
    for position, mod in enumerate(modification_order):
        for match in molecule.split(";"):
            ...
            output_strings.append(
                "{}:{}[{}]".format(
                    split[0], int(split[1]) + modification_positions[position], mod
                )
            )

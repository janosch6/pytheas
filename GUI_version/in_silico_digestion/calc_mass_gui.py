...
class Masses:
    def __init__(
        ...
        CID_series,
        mz_consolidation,
        MS1_consolidation_ppm,
        MS2_consolidation_ppm,
        fasta_name,
    ):
        self.ion_mode, self.nts_alphabet_light, self.nts_alphabet_heavy = (
            ion_mode,
            light_alphabet,
            heavy_alphabet,
        )
        ...
        for b in unique_seq:
            ...
            if line.split()[1] == "light":
                output_string = "{}({}1):{}".format(
                    ...
                    self.ion_mode,
                    str(
                        round_masses(self.nts_dic_onlyB_light[b] + freebase_correction)
                    ),
                )
                ...
                if charge_prec > 1:
                    ...
                    if chem3prime == "P" or chem5prime == "P":
                        ...
                        MBP_mz = (
                            np.float64(line.split()[0]) * charge_prec
                            + charge_correction
                            - self.nts_dic_onlyB_light[last_nt]
                            + self.dic_M_x_series["M-PO3H"]
                        ) / charged_loss
                ...
                MBH_mz = (
                    np.float64(line.split()[0])
                    - (self.nts_dic_onlyB_light[b] + ele_mass["H"]) / charge_prec
                )
                ...
                output_masses.append(
                    "{}({}1):{}".format(
                        ...
                        self.ion_mode,
                        str(
                            round_masses(self.nts_dic_onlyB_heavy[b])
                            + freebase_correction
                        ),
                    )
                )
                ...
                if charge_prec > 1:
                    ...
                    if chem3prime == "P" or chem5prime == "P":
                        ...
                        MBPh_mz = (
                            np.float64(line.split()[0]) * charge_prec
                            + charge_correction
                            - self.nts_dic_onlyB_heavy[last_nt]
                            + self.dic_M_x_series["M-PO3H"]
                        ) / charged_loss
                ...
                if str(i) in MS2_charge_dic.keys():
                    ...
                    if line.split()[1] == "light":
                        ...
                        if ion == "y-P" or ion == "z-P":
                            mass_fragment += (
                                ...
                                + self.dic_CID_series_masses[ion[0]]
                            )

                        else:
                            mass_fragment += (
                                nts_dic_light[sequence[i - 1]]
                                + self.dic_CID_series_masses[ion]
                            )
                    ...
                    if (
                        ...
                        # Add the 3'-P end mass to the fragment
                            ...
                            if ion == "y-P" or ion == "z-P":
                                mass_fragment_3OH = (
                                    mass_fragment + self.dic_CID_series_masses[ion]
                                )
                    ...
                    for char in MS2_charge_dic[str(i)]:
                        ...
                        if int(char) <= charge_prec:
                            ...
                            # Keep the ion if its m/z is in the given interval for MS2 ions
                            if (
                                self.MS2_mzhigh >= mz_fragment >= self.MS2_mzlow
                                and ion != "y-P"
                                and ion != "z-P"
                            ):
    ...
    def final_lines_MS2(self, lines_MS2):
        ...
        for line in lines_MS2:
            output_lines.append(
                " ".join(
                    line.split()
                    + self.fragment_MS2_masses(
                        line, self.MS2_charge_table, nts_dic_heavy
                    )
                    + list("\n")
                )
            )
    ...
    def header_info_MS2(self, input_file):
        ...
        header_lines.append(
            "#CID_SERIES " + " ".join(sorted(list(set(self.CID_series)))) + "\n"
        )
        ...
        header_lines.append("#MZHIGH_MS1 " + str(self.MS1_mzhigh) + "\n")
        header_lines.append(
            "#ELEMENTAL_COMPOSITION_LIGHT " + str(self.nts_alphabet_light) + "\n"
        )

        if self.nts_alphabet_heavy:
            header_lines.append(
                "#ELEMENTAL_COMPOSITION_HEAVY " + str(self.nts_alphabet_heavy) + "\n"
            )
        ...
        if self.mz_consolidation:
            header_lines.append(
                "#MS1_SEQX_PPM " + str(self.MS1_consolidation_ppm) + "\n"
            )
            header_lines.append(
                "#MS2_SEQX_PPM " + str(self.MS2_consolidation_ppm) + "\n"
            )
    ...
    def final_output(self):
        ...
        if os.path.exists("./output.3.MS1"):
            ...
            open("./Digest_MS1.txt", "w").writelines(
                ...
                + self.lines_final(
                    self.lines_with_charge(
                        lines_with_chemistry(inlines_MS1(open("./output.3.MS1", "r")))
                    ),
                    self.MS1_mzlow,
                    self.MS1_mzhigh,
                )
        ...
        if os.path.exists("./output.3.MS2"):
            ...
            self.nts_dic_onlyB_light = nts_mass(
                self.read_excel_input(self.nts_alphabet_light)
            )[1]

            if self.nts_alphabet_heavy:
                self.nts_dic_onlyB_heavy = nts_mass(
                    self.read_excel_input(self.nts_alphabet_heavy)
                )[1]

            lines_MS2 = []

            for line in self.lines_final(
                self.lines_with_charge(lines_with_chemistry(inlines_MS2())),
                self.MS1_mzlow,
                self.MS1_mzhigh,
            ):
            ...
            open(os.getcwd() + "/Digest_" + output_name + ".txt", "w").writelines(
        ...
        if self.mz_consolidation:
            digest_lines = ct.mz_consolidate(
                self.nts_alphabet_light,
                "Digest_" + output_name + ".txt",
                "light",
                self.MS1_consolidation_ppm,
                self.MS2_consolidation_ppm,
                read_csv(),
            )
            if digest_lines:

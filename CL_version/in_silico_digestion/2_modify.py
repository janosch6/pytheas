...
parser.add_argument(
    "--mod_profile", help="File with molecule-specific RNA modifications (Optional)"
)
parser.add_argument(
    "--nts_light",
    required=True,
                ...
                if seq.split()[0] == mod.split()[0]:
                    if (
                        ...
                        print(
                            ...
                            "be modified anyway, please check the input files.".format(
                                mod,
                                list(seq.split()[1])[int(mod.split()[1]) - 1],
                                mod_origin[mod.split()[2]],
                            )
    ...
    for modline in profile_mod:
        ...
        if int(modline.split()[-1]) > 0:
            ...
            for i, line in enumerate(input_lines):
                if (
                    int(modline.split()[1]) >= int(line.split()[2])
                    and int(modline.split()[1]) <= int(line.split()[3])
                    ...
                    if modline.split()[-1] == "1":
                        ...
                        if "@" in split[-1]:
                            ...
                            s[int(modline.split()[1]) - int(split[2])] = mod_alphabet[
                                modline.split()[2]
                            ]
...
if __name__ == "__main__":
    ...
    if args.mod_profile:
        input_lines = mod_0_1_2_mode(
            profile_mod(os.getcwd() + "/" + args.mod_profile),
            lines,
            read_excel_input()[0],
        )
    else:
    ...
    open(os.getcwd() + "/output.2", "w").writelines(
        unique_list(
            output_file(
                mod_nts_exceptions(
                    input_lines, read_excel_input()[2], read_excel_input()[0]
                ),
                os.getcwd() + "/output.1",
            )
        )

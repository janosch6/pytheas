...
parser.add_argument(
    "--enzyme",
    choices=["A", "T1", "U2", "none", "nonspecific", "Cus", "MC1", "MAZ", "custom"],
    required=True,
...
parser.add_argument(
    "--custom_enzyme",
    default=None,
    help="Input file with custom enzymatic cleavage sites",
)
parser.add_argument(
    "--miss",
    type=int,
...
def print_ReSites(id, sequence, enzyme):
    ...
    if enzyme == "none":
        ...
        output_lines.append(
            "{} {} {} {} {}\n".format(id, sequence, str(1), str(len(sequence)), str(0))
        )
        ...
        if enzyme == "custom":
            if not args.custom_enzyme:
                print("ERROR!! Select an input file with the custom cleaving sites")
                sys.exit(1)
            else:
                        ...
                        # Append the missed cleavages lines based on the selected values
                            ...
                            if args.miss == 0:
                                ...
                                final_lines = final_lines + clean_lines(
                                    miss_1(
                                        print_ReSites(
                                            str(seq.id), sequence, args.enzyme
                                        )
                                    )
                                )
...
if __name__ == "__main__":
    ...
    if args.enzyme == "custom":
        if not args.custom_enzyme:
            ...
            cleavages = (
                f"#CLEAVING_SITES {','.join(read_custom_enzyme(args.custom_enzyme))}\n"
            )
    ...
    if args.enzyme == "nonspecific":
        starting_lines = [
            ...
            "{}\n"
            "molecule sequence residue_start residue_end miss 5'end 3'end\n".format(
                os.path.basename(args.RNA_sequences[0]),
                args.enzyme,
                args.nonspecific_min_length,
                args.nonspecific_max_length,
                ",".join(args.cleaved_fragments_5end_chem),
                ",".join(args.cleaved_fragments_3end_chem),
    ...
    open(os.getcwd() + "/output.1", "w").writelines(
        starting_lines + list(set(generate_output()))
    )
